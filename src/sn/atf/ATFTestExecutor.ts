...
export class ATFTestExecutor {
	...
	async executeTest(testId: string): Promise<TestResult | undefined> {
	...
	private async waitForTestCompletion(progressId: string) {
		let pr: ProgressResult | undefined = await this.doGetTestProgress(progressId);
	...
	private async doGetTestProgress(progressId: string) {
	...
	private async getTestProgress(progressId: string) {
	...
	private async getTestResult(testResultId: string): Promise<TestResult | undefined> {
		...
		const resp = await this._req.get<ServiceNowTableResponse<TestResult>>(request);
		if (resp.status == 200) {
