...
export class Application {
	...
	public constructor(instance: IServiceNowInstance, scope: string, applicationId: string) {
	...
	private async getAppAndSummary() {
		let userSession = await this.snRequest.getUserSession();
		if (userSession) {
			const { appID, sys_class_name, upgrade_finished } = await getAppAndSummary(this._applicationId, this._scope, userSession as any);
