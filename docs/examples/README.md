...
```bash
# Tail syslog
npx tsx docs/examples/syslog-tail-cli.ts --instance your-instance

# Tail error logs only
npx tsx docs/examples/syslog-tail-cli.ts --instance your-instance --level error

# Tail with output file
npx tsx docs/examples/syslog-tail-cli.ts --instance your-instance --output ./logs/tail.log

# Tail app scope logs
npx tsx docs/examples/syslog-tail-cli.ts --instance your-instance --table syslog_app_scope
```

### Option 2: Using Node with ts-node/esm loader

```bash
node --loader ts-node/esm docs/examples/syslog-tail-cli.ts --instance your-instance --level error
```
...
node docs/examples/syslog-tail-cli.js --instance your-instance
```
...
npx tsx docs/examples/syslog-tail-cli.ts --instance your-instance --query "sourceSTARTSWITHsecurity"

# Monitor and save to file
npx tsx docs/examples/syslog-tail-cli.ts --instance your-instance --level error --output ./error-logs.txt

# Monitor specific app scope
npx tsx docs/examples/syslog-tail-cli.ts --instance your-instance --table syslog_app_scope --query "app_scope=x_my_app"

# Fast polling (every 2 seconds)
npx tsx docs/examples/syslog-tail-cli.ts --instance your-instance --interval 2000
```
...
2. **Use node with loader:**
   ```bash
   node --loader ts-node/esm docs/examples/syslog-tail-cli.ts --instance your-instance
   ```
...
```bash
snc configure profile set --profile your-instance --username <username> --password <password> --instanceUrl <url>
```
