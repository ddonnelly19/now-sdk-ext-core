...
```bash
node docs/examples/syslog-tail-cli.js --instance your-instance --level error
```
...
const instanceAlias = process.argv[2] || 'your-instance';
...
**Run it:**
```bash
node syslog-tail.mjs your-instance
```
...
async function main() {
    const instanceAlias = 'your-instance'; // Or get from args
    const credential = await getCredentials(instanceAlias);
