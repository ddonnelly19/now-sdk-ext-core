...
describe('ATFTestExecutor', () => {
	...
	describe('execute test', () => {
		...
		xit('defined test', async () => {
			...
			const resp = await testExec.executeTest('60ecb62b933626505ecb74718bba10e8');
	...
	describe('execute test suite', () => {
		...
		it('should execute test suite by sys_id with CICD Credentials', async () => {
			const testInstanceUrl = process.env.TEST_SN_INSTANCE_URL;
			const testPassword = process.env.TEST_SN_PASSWORD;
			if (!testInstanceUrl || !testPassword) {
				return; // skip: set TEST_SN_INSTANCE_URL and TEST_SN_PASSWORD env vars to run
			}

			process.env['SN_SDK_NODE_ENV'] = 'SN_SDK_CI_INSTALL';
			process.env['SN_SDK_INSTANCE_URL'] = testInstanceUrl;
			process.env['SN_SDK_USER'] = 'admin';
			process.env['SN_SDK_USER_PWD'] = testPassword;
			...
			expect(cred.instanceUrl).toBe(testInstanceUrl);
			expect(cred.username).toBe('admin');
			expect(cred.password).toBe(testPassword);
